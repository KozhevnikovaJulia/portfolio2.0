...
export function ContactForm(props: any) {
  function sendEmail(e: any) {
    ...
    emailjs.sendForm('service_bjbi15g', 'template_shjo7bi', e.target, 'user_eQFOSWwvznTDIbhzcNLDU').then(
      result => {
  ...
  return (
    <form onSubmit={sendEmail} className={style.form}>
      <div>
        <InputContactForm name='name' placeholder={'Name'} />
      </div>
      <div>
        <InputContactForm name='email' placeholder={'Email'} />
      </div>
      <div>
        <InputContactForm name='subject' placeholder={'Subject'} />
      </div>
      <div>
        <TextareaContactForm name='message' placeholder={'Message'} />
      </div>
      <div>
        <ButtonContactForm buttonText={'SEND'} />
      </div>
